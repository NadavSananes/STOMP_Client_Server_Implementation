...
std::string Event:: eventToString(std::string userName){
    std::string body = "user:" + userName + "\n" + "event name:" + name + "\n"
        + "time:" + std::to_string(time) + "\n" + "general game updates:\n";
    
    std::string generalGameUpdates = "";

    for (std::map<std::string, std::string>::iterator it=game_updates.begin(); it!=game_updates.end(); ++it){
        generalGameUpdates = generalGameUpdates + "    " + it->first + ":" + it->second + "\n";
    }

    std::string teamAUpdates = "";

    for (std::map<std::string, std::string>::iterator it=team_a_updates.begin(); it!=team_a_updates.end(); ++it){
        teamAUpdates = teamAUpdates + "    " + it->first + ":" + it->second + "\n";
    }

    std::string teamBUpdates = "";

    for (std::map<std::string, std::string>::iterator it=team_b_updates.begin(); it!=team_b_updates.end(); ++it){
        teamBUpdates = teamBUpdates + "    " + it->first + ":" + it->second + "\n";
    }

    body = body + generalGameUpdates + "team a updates:\n" + teamAUpdates + "team b updates:\n" + teamBUpdates;
